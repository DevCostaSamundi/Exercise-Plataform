    ...
    <div className="space-y-4">
      ...
      <form onSubmit={handleSubmitComment} className="flex gap-3">
        ...
        <button
          ...
          className="px-4 py-2 bg-black text-white rounded-lg font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
        >
          ...
          {comments.map((comment) => (
            <div key={comment._id} className="flex gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {/* Replies (apenas visualização — endpoint de criação não existe ainda) */}
                    ...
                    {comment.replies.map((reply) => (
                      <div key={reply._id} className="flex gap-2">
                        ...
                        <div className="flex-1 bg-slate-50 dark:bg-slate-900 rounded-lg p-2">
                          ...
                          <p className="text-slate-700 dark:text-slate-300 text-xs">{reply.content}</p>
                        </div>
                      </div>
                    ))}
                  </div>
