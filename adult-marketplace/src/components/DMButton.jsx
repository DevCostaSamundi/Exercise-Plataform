...
export default function DMButton({ creatorId }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!user) {
          setAllowed(false);
          setMessage(cfg.dmPolicy === 'subscribers' ? 'Somente assinantes Premium' : 'Dá um tip para desbloquear o chat');
          return;
        }

        if (cfg.dmPolicy === 'subscribers') {
          const res = await api.get(`/api/v1/users/${user.id}/subscription-status?creatorId=${creatorId}`);
        ...
        if (cfg.dmPolicy === 'tippers') {
          const r = await api.get(`/api/v1/users/${user.id}/has-tipped?creatorId=${creatorId}`);
  ...
  const openChat = async () => {
    ...
    try {
      ...
      // navigate to chat page (use your router)
